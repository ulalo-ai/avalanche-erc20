...
async function deployFaucet() {
  console.log("Deploying Updated UlaloTokenFaucet...");
  const wallet = getWallet("ulalo");  // Changed to use ulalo network
  console.log("Deployer address:", wallet.address);

  // Use the specific token address
  const ulaloTokenAddress = "0xA916910eDCA3639F42E0E8b397fBBEaf2412Fbe1";
  console.log("Using UlaloToken at:", ulaloTokenAddress);
  ...
  try {
    ...
    console.log(`View contract: https://explorer.ulalo.xyz/address/${faucetAddress}`);
  } catch (error) {
  ...
  console.log(`Explorer Link: https://explorer.ulalo.xyz/address/${faucetAddress}`);
  console.log("----------------------------");
  ...
  const deploymentInfo = {
    ...
    explorerLink: `https://explorer.ulalo.xyz/address/${faucetAddress}`
  };

  fs.writeFileSync(
    'ulalo-faucet-deployment-ulalo.json', 
    JSON.stringify(deploymentInfo, null, 2)
  );
  console.log("Deployment info saved to ulalo-faucet-deployment-ulalo.json");
